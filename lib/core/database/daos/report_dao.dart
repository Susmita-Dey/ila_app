...
class ReportDao extends DatabaseAccessor<AppDatabase> with _$ReportDaoMixin {
  ...
  Future<DoctorReportData> generateReport({
    ...
    // NOTE: using .year, .month, .day to guarantee bounds for sqlite
    ...
    // 5. PMDD Clustering
    Map<String, Map<String, int>> symptomPhases = {}; // symptom -> {Luteal: x, Menstrual: y, Mid: z}

    for (var event in cycles) {
      if (event.symptoms == null || event.symptoms!.isEmpty) continue;
      
      // Find current and next true cycle
    ...
    symptomPhases.forEach((symp, counts) {
      ...
      if (total > 0) {
        ...
        if (ratio >= 0.70 && dominantPhase == 'Luteal Phase (PMDD Indicator)') {
          clusterLabel = '⚠️ Luteal Clustering (PMDD)';
    ...
    for (int i = 0; i < limit; i++) {
      ...
      // Aggregate flow descriptions for this cycle (from c.date to nextCycle.date)
      ...
      cycleRows.add([
        (chronIndex + 1).toString(), // Cycle number
        formatter.format(c.date), // Start Date
        endDateStr, // End Date
        lengthStr, // Length
        flowDesc, // Flow
        allSymptoms.isNotEmpty ? allSymptoms.join(', ') : 'None', // Symptoms
      ]);
    }
